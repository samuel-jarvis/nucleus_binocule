...
const ObjectFunctionList = ({ id }: { id: string }) => {
  ...
  return <div className="mt-10">
    <h3 className="text-2xl font-semibold text-black mb-2 border-b">Object Functions</h3>
      ...
      {data.map((item: ICreateFunctionOrActivity) => (
        <div key={item.title} className="bg-white  rounded-lg border-b">
          ...
          {item.attributes && item.attributes.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-blue mb-2">Attributes</h3>
              <ul className="list-disc pl-5">
                {item.attributes.map((attr, index) => (
                  <li key={index} className="mb-2">
                    ...
                    {attr.type && (
                      <span className="text-gray-500 text-sm ml-1">({attr.type})</span>
                    )}
                    {attr.example && (
                      <p className="text-gray-500 text-xs mt-1">Example: {attr.example}</p>
                    )}
                  </li>
                ))}
              </ul>
